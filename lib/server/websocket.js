        ...
        for (const { file, fullPath } of pdfFiles) {
          ...
          try {
            ...
            if (existingForms.length === 0) {
              ...
              console.log(`Form already exists: ${file} (ID: ${existingForm.id})`);
              
              // Check if the existing form has any pages
              const existingPages = await db.getFormPages(existingForm.id);
              console.log(`Existing form has ${existingPages.length} pages`);
              
              // Check if pages are placeholders (small size or specific content)
              const hasPlaceholderImages = existingPages.some(page => {
                const imageData = page.image_data || '';
                return imageData.length < 1000 || imageData.includes('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1Pe');
              });
              
              if (existingPages.length === 0 || hasPlaceholderImages) {
                console.log(`Reprocessing PDF for existing form: ${file} (placeholder images detected)`);
                // Delete existing placeholder pages first
                if (existingPages.length > 0) {
                  const deletedPages = await db.deleteFormPages(existingForm.id);
                  console.log(`Deleted ${deletedPages.length} placeholder pages`);
                }
                
                try {
                  const success = await processPdfFile(fullPath, existingForm.id);
                  if (success) {
                    console.log(`✅ Successfully reprocessed PDF: ${file}`);
                  } else {
                    console.error(`❌ Failed to reprocess PDF: ${file}`);
                    // Create a placeholder page as fallback
                    await db.createFormPage(existingForm.id, 1, 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGMAAQAABQABDQottAAAAABJRU5ErkJggg==');
                  }
                } catch (processError) {
                  console.error(`Error reprocessing PDF file ${fullPath}:`, processError);
                  // Create a placeholder page as fallback
                  await db.createFormPage(existingForm.id, 1, 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGMAAQAABQABDQottAAAAABJRU5ErkJggg==');
                }
              } else {
                console.log(`Form already has valid pages, skipping reprocessing`);
              }
              
              // Broadcast existing form to client to refresh it
              this.broadcastMessage({ type: 'add', form: existingForm });
            }
          } catch (error) {
